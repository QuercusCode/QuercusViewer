        ...
        <div className="w-48 shrink-0 space-y-1">
                ...
                <div key={c.id} className="group relative">
                    {renamingId === c.id ? (
                        <div className="flex items-center gap-1 px-2">
                            ...
                            <button onClick={() => setRenamingId(null)} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] p-0.5">
                                <X className="w-3 h-3" />
                            </button>
                        </div>
                    ) : (
                        <button onClick={() => onSelect(c.id)}
                            className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-sm transition-all
                                ${activeCollection === c.id ? 'bg-[var(--input-bg)] text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:bg-[var(--input-bg)] hover:text-[var(--text-primary)]'}`}>
                            <span className={`w-2 h-2 rounded-full shrink-0 ${DOT[c.color] ?? 'bg-neutral-400'}`} />
                            <span className="truncate flex-1 text-left">{c.name}</span>
                            <span className="text-[10px] text-[var(--text-muted)]">{counts[c.id] ?? 0}</span>
            ...
            {creating ? (
                ...
                <button onClick={() => setCreating(true)}
                    className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-[var(--input-bg)] border border-dashed border-[var(--border-main)] hover:border-[var(--border-main)] transition-all">
