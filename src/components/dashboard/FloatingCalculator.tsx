...
export const FloatingCalculator: React.FC<FloatingCalculatorProps> = ({ onClose }) => {
  const [type, setType] = useState<'dilution' | 'molarity'>('dilution');
  ...
  const toggleType = (newType: 'dilution' | 'molarity') => {
      ...
      <div className="p-3 border-b border-[var(--border-main)] bg-[var(--bg-main)]/30">
        <div className="flex bg-[var(--bg-main)] p-0.5 rounded-lg border border-[var(--border-main)] shadow-inner">
          <button 
            ...
            className={`flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all ${type === 'dilution' ? 'bg-blue-500/10 text-blue-400 shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
          >
            Dilution
          </button>
          <button 
            onClick={() => toggleType('molarity')}
            className={`flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all ${type === 'molarity' ? 'bg-blue-500/10 text-blue-400 shadow-sm' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
          >
            Molarity
          </button>
        </div>
